...
from agents.bc_agent import SoftModuleAgent, MLPAgent, MLPEmbeddingAgent, MHSACAgent
...
from metaworld_utils.customize_env_dict import ENV_TASK_DICT
...
class BC_Trainer(object):
    ...
    def __init__(self, args, params):
        ...
        self.args = args
        self.args['agent_class'] = SoftModuleAgent ## HW1: you will modify this
        ...
        if self.params["env_name"] == "mt10":
        ...
        elif self.params["env_name"] in ENV_TASK_DICT.keys():
            CLS_DICT = ENV_TASK_DICT[self.params["env_name"]]
            for name, env_name in CLS_DICT.items():
                # add necessary env args
                expert_env = get_meta_env(env_name, params['env'], params['meta_env'], return_dicts=False) 
                expert_env.seed(args["seed"])
                params['expert_net']['base_type']=MLPBase
            ...
            raise NotFoundErr("the env name {} is not found\n" %self.params["env_name"])
...
def plot_success_curve(agent_curve_1, agent_curve_2, agent_curve_5, agent_curve_10, eval_interval, env_name):
    ...
    ax.set_title("Soft-Module Sample-SuccessRate Curve")
    ax.set(ylim=(-0.1, 1.1))
    
    fig = ax.get_figure()
    fig.savefig("./fig/" + env_name +"_soft_module_sample_and_success_curve.png")
